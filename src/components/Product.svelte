    ...
    <div class="flex flex-col gap-10 sm:gap-14 md:gap-24 flex-1 items-center justify-center pb-10 md:pb-14">
        <div class = "flex flex-col gap-2">
            <p class="opacity-60 text-base sm:text-lg md:text-xl text-center">
                Start your lifetime <span class="text-blue-400">gym streak</span> today.
            </p>
        ...
        {#each productFeatures as productFeature, i}
            <ProductCard productFeature={productFeature} {i}>
                {#if i === 0}
                    Experience a beginner exercise <span class=text-blue-400> master class.</span>
                {:else if i === 1}
                    Lay the foundations for <span class=text-blue-400> routine and consistency.</span>
                {:else}
                    Workouts specific to your <span class=text-blue-400> situation and experience.</span>
                {/if}
            </ProductCard>
        {/each}
    </div>
